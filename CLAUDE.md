...
- Compile-time i18n via Paraglide — zero runtime i18n library in client bundle
- Message source files: `messages/{locale}.json` (17 locales, namespace-prefixed keys)
- Generated typed functions: `src/paraglide/messages.ts` (auto-generated by `bun run i18n:compile`)
- Usage: `import * as m from "@/paraglide/messages"` then `m.app_cancel()`, `m.common_home()`
- Locale management: `languageTag()` / `setLanguageTag()` from `@/paraglide/runtime`
- Locale detection: cookie (NEXT_LOCALE) -> Accept-Language header -> default (en) in middleware
